...
function syncLocalSettings() {
  if (settings.value) {
    const stealFilter = (settings.value as any).stealFilter || { enabled: false, mode: 'blacklist', plantIds: [] }
    const stealFriendFilter = (settings.value as any).stealFriendFilter || { enabled: false, mode: 'blacklist', friendIds: [] }
    ...
    localSettings.value = JSON.parse(JSON.stringify({
      ...
      stealFilter: {
        enabled: stealFilter.enabled || false,
        mode: stealFilter.mode || 'blacklist',
        plantIds: Array.isArray(stealFilter.plantIds) ? stealFilter.plantIds : [],
      },
      stealFriendFilter: {
        enabled: stealFriendFilter.enabled || false,
        mode: stealFriendFilter.mode || 'blacklist',
        friendIds: Array.isArray(stealFriendFilter.friendIds) ? stealFriendFilter.friendIds : [],
      },
    }))
...
const selectedPlantIds = computed({
  get: () => {
    const ids = localSettings.value.stealFilter.plantIds
  ...
  set: (value) => {
    localSettings.value.stealFilter.plantIds = Array.from(value) as any
  }
})

const selectedFriendGids = computed({
  get: () => {
    const ids = localSettings.value.stealFriendFilter.friendIds
    if (Array.isArray(ids)) {
      return new Set(ids.map(id => Number(id)))
    }
    return new Set()
  ...
  set: (value) => {
    localSettings.value.stealFriendFilter.friendIds = Array.from(value) as any
  }
})

function togglePlantId(plantId: number) {
  const current = new Set(selectedPlantIds.value)
  ...
  selectedPlantIds.value = current
}

function toggleFriendGid(gid: number) {
  const current = new Set(selectedFriendGids.value)
  if (current.has(gid)) {
    current.delete(gid)
  }
  else {
  ...
  selectedFriendGids.value = current
}
...
async function handleChangePassword() {
  ...
  try {
    ...
    if (isAdmin.value) {
      // 管理员使用旧的接口
      res = await settingStore.changeAdminPassword(passwordForm.value.old, passwordForm.value.new)
    }
    else {
      // 普通用户使用新的接口
      res = await api.post('/api/auth/change-password', {
    ...
    showAlert(`修改失败: ${e.response?.data?.error || e.message || '未知错误'}`, 'danger')
  }
  finally {
...
async function handleSaveSettings() {
  ...
  try {
    const settingsToSave = {
      ...localSettings.value,
      stealFilter: {
        enabled: localSettings.value.stealFilter.enabled,
        mode: localSettings.value.stealFilter.mode,
        plantIds: selectedPlantIds.value.size > 0 ? Array.from(selectedPlantIds.value) : [],
      },
      stealFriendFilter: {
        enabled: localSettings.value.stealFriendFilter.enabled,
        mode: localSettings.value.stealFriendFilter.mode,
        friendIds: selectedFriendGids.value.size > 0 ? Array.from(selectedFriendGids.value) : [],
      },
    }

    await api.post('/api/settings/save', settingsToSave)
    showAlert('偷菜过滤设置已保存')
  }
  catch (e: any) {
...
<template>
  <div class="settings-page">
    ...
    <div v-else class="grid grid-cols-1 mt-12 gap-4 text-sm lg:grid-cols-2">
      ...
      <!-- Steal Filter Column -->
      <div class="flex h-full flex-col bg-white shadow-sm sm:rounded-lg dark:bg-gray-800 sm:border sm:border-gray-200 dark:sm:border-gray-700">
        <!-- Steal Filter Header -->
        <div class="border-b bg-gray-50/50 px-4 py-3 dark:border-gray-700 dark:bg-gray-800/50">
          <h3 class="flex items-center gap-2 text-base text-gray-900 font-bold dark:text-gray-100">
            <div class="i-carbon-filter-edit" />
            偷菜过滤
          </h3>
        </div>

        <!-- Steal Filter Content -->
        <div class="flex-1 space-y-3 overflow-y-auto p-4">
          <!-- 偷植物过滤 -->
          <div class="space-y-3">
            <h4 class="text-sm font-medium text-gray-900 dark:text-white">植物过滤</h4>
            <BaseSwitch
              v-model="localSettings.stealFilter.enabled"
              label="启用植物过滤"
            />
            <div v-if="localSettings.stealFilter.enabled" class="space-y-3">
              <BaseSelect
                v-model="localSettings.stealFilter.mode"
                label="过滤模式"
                :options="[
                  { value: 'blacklist', label: '黑名单（不偷列表中的）' },
                  { value: 'whitelist', label: '白名单（只偷列表中的）' },
                ]"
              />
              <div>
                <label class="block text-sm text-gray-700 dark:text-gray-300 mb-2">选择植物</label>
                <div class="max-h-48 overflow-y-auto space-y-1 border border-gray-200 rounded-lg p-2 dark:border-gray-600">
                  <div v-if="!seeds || seeds.length === 0" class="text-sm text-gray-500 text-center py-2">
                    暂无种子数据
                  </div>
                  <label
                    v-for="seed in seeds"
                    :key="seed.seedId"
                    class="flex items-center gap-2 p-1.5 hover:bg-gray-50 dark:hover:bg-gray-700 rounded cursor-pointer"
                  >
                    <input
                      type="checkbox"
                      :checked="selectedPlantIds.has(seed.seedId)"
                      @change="togglePlantId(seed.seedId)"
                      class="rounded border-gray-300"
                    >
                    <span class="text-sm text-gray-700 dark:text-gray-300">
                      {{ seed.name }} (ID: {{ seed.seedId }})
                    </span>
                  </label>
                </div>
                <p class="text-xs text-gray-500 mt-1">
                  已选择 {{ selectedPlantIds.size }} 个植物
                </p>
              </div>
            </div>
          </div>
          ...
          <!-- 偷用户过滤 -->
          <div class="space-y-3">
            <h4 class="text-sm font-medium text-gray-900 dark:text-white">好友过滤</h4>
            <BaseSwitch
              v-model="localSettings.stealFriendFilter.enabled"
              label="启用好友过滤"
            />
            <div v-if="localSettings.stealFriendFilter.enabled" class="space-y-3">
              <BaseSelect
                v-model="localSettings.stealFriendFilter.mode"
                label="过滤模式"
                :options="[
                  { value: 'blacklist', label: '黑名单（不偷列表中的）' },
                  { value: 'whitelist', label: '白名单（只偷列表中的）' },
              ...
              />
              <div>
                <label class="block text-sm text-gray-700 dark:text-gray-300 mb-2">选择好友</label>
                <div class="max-h-48 overflow-y-auto space-y-1 border border-gray-200 rounded-lg p-2 dark:border-gray-600">
                  <div v-if="loadingFriends" class="text-sm text-gray-500 text-center py-2">
                    加载中...
                  </div>
                  <div v-else-if="!friends || friends.length === 0" class="text-sm text-gray-500 text-center py-2">
                    暂无好友数据
                  </div>
                  <label
                  ...
                  >
                    <input
                      ...
                      @change="toggleFriendGid(friend.gid)"
                      class="rounded border-gray-300"
                    >
                    <span class="text-sm text-gray-700 dark:text-gray-300">
                      {{ friend.name || `好友 ${friend.gid}` }} (GID: {{ friend.gid }})
                    </span>
                  </label>
                </div>
                <p class="text-xs text-gray-500 mt-1">
                  已选择 {{ selectedFriendGids.size }} 个好友
                </p>
              </div>
            </div>
          </div>
