...
async function handleClearLogs() {
  ...
  const accountId = currentAccountId.value || 'all'
  const success = await statusStore.deleteLogs(accountId)

  if (success) {
    // 刷新日志列表
    if (accountId && accountId !== 'all') {
      await statusStore.fetchLogs(accountId, {})
    }
    // 清空前端显示的日志
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3 sm:grid-cols-2">
      ...
      <div class="flex flex-col rounded-lg bg-white p-4 shadow dark:bg-gray-800">
        <div class="mb-2 flex items-start justify-between">
          ...
          <div class="rounded px-2 py-0.5 text-xs dark:px-2 dark:py-0.5" style="background-color: color-mix(in srgb, var(--theme-primary) 15%, var(--color-bg-white, white)); color: var(--theme-primary)">
        ...
        <div class="mt-auto">
          <div class="mb-1 flex justify-between text-xs text-gray-500">
            <div class="flex items-center gap-1">
              <div class="i-fas-bolt" style="color: var(--theme-primary)" />
              <span>EXP</span>
            </div>
