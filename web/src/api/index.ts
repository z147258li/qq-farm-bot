...
const tokenRef = useStorage('auth_token', '')
const adminTokenRef = useStorage('admin_token', '')
...
api.interceptors.request.use((config) => {
  // 使用新的用户token或旧的admin token
  const token = tokenRef.value || adminTokenRef.value
  if (token) {
...
api.interceptors.response.use((response) => {
  ...
  if (error.response) {
    if (error.response.status === 401) {
      ...
      toast.error(error.response.data?.error || '权限不足')
    }
